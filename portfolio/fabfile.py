...
if not hasattr(env, "proj_app"):
    env.proj_app = "portfolio"

conf = {}
if sys.argv[0].split(os.sep)[-1] in ("fab", "fab-script.py"):
    # Ensure we import settings from the current dir
    try:
        conf = import_module("%s.settings.production" % env.proj_app).FABRIC
        try:
            conf["HOSTS"][0]
...
env.admin_login = conf.get("ADMIN_LOGIN", "admin")
...
env.proj_path = "/home/%s/wagtail/%s" % (env.user, env.proj_name)
...
def rsync_upload():
    ...
    """
    excludes = ["*.pyc", "*.pyo", "*.db", ".DS_Store", ".coverage", "/locale",
                "prod.py", "/media", "/.git", "/.hg"]
...
def python(code, show=True):
    ...
    """
    setup = "import os;" \
            "os.environ[\'DJANGO_SETTINGS_MODULE\']=\'%s.settings.production\';" \
            "import django;" \
...
def secure(new_user=env.user):
    ...
    sudo("adduser --gecos '' %s" % new_user)
    sudo("usermod -G sudo %s" % new_user)
    sudo("sed -i 's:RootLogin yes:RootLogin no:' /etc/ssh/sshd_config")
...
def install():
    ...
    apt("nginx libjpeg-dev python-dev python-setuptools git-core "
        ...
        "wkhtmltopdf xvfb libz-dev libjpeg-dev libfreetype6-dev python3-dev")
